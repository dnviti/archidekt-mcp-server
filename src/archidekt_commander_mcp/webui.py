  ...
  <script>
    ...
    function buildInstructions(collection, prompt) {{
      ...
      return [
        ...
        "- When you need sorting, prefer canonical filters with `sort_by` plus `sort_direction`.",
        "- Use `sort_by: unit_price` and `sort_direction: desc` for highest price first.",
        "- Use `sort_by: unit_price` and `sort_direction: asc` for cheapest cards first.",
        "- Use `sort_by: edhrec_rank` and `sort_direction: asc` for the best EDHREC rank first.",
        "- Use `sort_by: rarity` or `sort_by: added_at` only when those are actually relevant to the request.",
        ...
        "- Deck writes may use `modifications.quantity` values greater than 1 when needed.",
        "- For deck writes, quantity belongs in `modifications.quantity`, not as a top-level card field.",
        "- For collection writes, provide `record_id` to update an existing row; omitting it creates a new row.",
        "- Use `delete_collection_entries` with `archidekt_record_ids` from `search_owned_cards` when removing collection rows.",
        "- After an authenticated collection write, the next authenticated read of that same self collection bypasses cached snapshots once.",
        "- For Commander decks, only basic lands should normally exceed 1 copy.",
        "- For most other formats, non-basic cards should normally stay at 4 copies or fewer while basic lands may be unlimited.",
        ...
        "- Do not default every card to 1; quantities should follow the deck format's copy rules.",
